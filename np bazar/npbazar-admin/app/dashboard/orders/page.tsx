...
export default function OrdersPage() {
  ...
  const fetchOrders = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      if (!opts?.silent) toastError((err as Error).message);
    } finally {
  ...
  // New orders and status changes push here live — refetch quietly, no spinner flash.
  useRealtimeRefresh(['order:new', 'order:updated'], useCallback(() => fetchOrders({ silent: true }), [fetchOrders]));
