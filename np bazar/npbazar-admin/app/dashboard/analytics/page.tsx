...
export default function AnalyticsPage() {
  ...
  // Unlimited-stock products (sellThroughRate === null — sell-through isn't
  // meaningful without a stock number) would otherwise render as broken/empty
  // rings in the radial chart, so they're excluded here rather than passed through.
  const sellThroughData = sellThrough
    .filter((p) => p.sellThroughRate !== null)
    .slice(0, 6)
    .map((p, i) => ({
  ...
  // Fallback when no product has stock tracking enabled (sell-through is
  // undefined for all of them) — show units sold instead of an empty chart.
  const soldFallbackData = sellThrough.slice(0, 6).map((p, i) => ({
    name: p.name.length > 14 ? p.name.slice(0, 14) + '…' : p.name,
    sold: p.sold,
    fill: PALETTE[i % PALETTE.length],
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className={cardCls}>
          ...
          ) : soldFallbackData.length > 0 ? (
            <>
              <p className="mb-2 text-xs text-zinc-400">
                No product has stock tracking enabled (all set to Unlimited), so sell-through rate can&apos;t be
                calculated — showing units sold instead.
              </p>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={soldFallbackData} layout="vertical" margin={{ left: 8, right: 24 }}>
                    <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                    <XAxis type="number" allowDecimals={false} />
                    <YAxis type="category" dataKey="name" width={110} tick={{ fontSize: 12 }} />
                    <Tooltip formatter={(v) => [v, 'Units sold']} />
                    <Bar dataKey="sold" radius={[0, 6, 6, 0]}>
                      {soldFallbackData.map((entry, i) => (
                        <Cell key={i} fill={entry.fill} />
                      ))}
                      <LabelList dataKey="sold" position="right" fontSize={12} fill="#3f3f46" />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={funnelData} layout="vertical" margin={{ left: 8, right: 24 }}>
                  ...
                  <Bar dataKey="value" radius={[0, 6, 6, 0]}>
                    ...
                    <LabelList dataKey="value" position="right" fontSize={12} fill="#3f3f46" />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
